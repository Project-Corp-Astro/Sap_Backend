 ...
 */
const createInternalError = (message, details = {}, originalError) => {
...
export async function getContentMetrics(req, res, next) {
    try {
        ...
        logger.error('Error fetching content metrics', { error: err.message });
        throw new AppError('Failed to fetch content metrics', ErrorTypes.INTERNAL, 500);
    }
}
/**
...
export async function getTimeSeriesData(req, res, next) {
    try {
        ...
        logger.error('Error fetching time series data', { error: err.message });
        next(createInternalError('Failed to fetch time series data', {}, err));
    }
}
/**
 ...
 * @param next - Express next middleware function
 */
export async function getCategoryDistribution(req, res, next) {
    try {
        ...
        logger.error('Error fetching category distribution', { error: err.message });
        next(createInternalError('Failed to fetch category distribution', {}, err));
    }
}
/**
 ...
 * @param next - Express next middleware function
 */
export async function getTopContent(req, res, next) {
    try {
        ...
        // Format top content
        const formattedData = topContent.map((item) => ({
        ...
        logger.error('Error fetching top content', { error: err.message });
        next(createInternalError('Failed to fetch top content', {}, err));
    }
}
/**
 ...
 * @param next - Express next middleware function
 */
export async function getAuthorPerformance(req, res, next) {
    try {
        ...
        // Format author data
        const formattedData = authorData.map((item) => ({
            id: item.id,
            name: item.name,
        ...
        logger.error('Error fetching author performance', { error: err.message });
        next(createInternalError('Failed to fetch author performance', {}, err));
    }
}
