 ...
 */
const ensureLogsDir = async (logsDir) => {
...
export function createLogger(config) {
    ...
    const { serviceName, level, maxSize, maxFiles, logsDir = 'logs', consoleLog, timestamps, timestampFormat } = mergedConfig;
    ...
    const baseFormat = timestamps
        ? combine(splat(), errors({ stack: true }), timestamp({ format: timestampFormat }), json())
        : combine(splat(), errors({ stack: true }), json());
    ...
    if (consoleLog && process.env.NODE_ENV !== 'production') {
        loggerTransports.push(new transports.Console({
            format: combine(colorize(), timestamps ? timestamp({ format: timestampFormat }) : format.simple(), consoleFormat),
        }));
    }
